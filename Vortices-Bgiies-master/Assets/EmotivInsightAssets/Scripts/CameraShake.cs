...
public class CameraShake : MonoBehaviour
{
    ...
    {
        isRunning = true;
        originalCameraPosition = Camera.main.transform.rotation;
        while (shakeDuration > 0.01f)
        {
            Vector3 rotationAmount = Random.insideUnitSphere * shakeAmount;//A Vector3 to add to the Local Rotation
            rotationAmount.z = 0;//Don't change the Z; it looks funny.
            rotationAmount.x = (Random.Range(-1,1) * shakeAmount) + 29.8f;
            ...
            if (smooth)
                ...
                Camera.main.transform.localRotation = Quaternion.Euler(rotationAmount);//Set the local rotation the be the rotation amount.

            yield return null;
        }
        Camera.main.transform.localRotation = originalCameraPosition;//Set the local rotation to 0 when done, just to get rid of any fudging stuff.
        isRunning = false;
    }
