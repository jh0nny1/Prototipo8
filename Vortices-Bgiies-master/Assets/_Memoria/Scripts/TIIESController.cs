...
namespace Memoria
{
	public class TIIESController : Visualization, IOnValidate, IOnDrawGizmos {

		public float rowDistanceDifference = 1.15f;
		...
		public override void CreateVisualization(bool createNewObjects, Vector3 planeCenter)
		{
			...
			for (var j = 0; j < elementsPerRow.Length; j++)
			{
				...
				for (var i = 0; i < elementsPerRow[j]; i++)            //por cada elemento de la fila
				{
					if (createNewObjects)
					{
						var grabableObject = Instantiate(InformationObjectManager.Instance.planeImages.informationPrefab, gameObject);     /* se instancia un DIOController en grabableObject */
		...
		private Vector3 RandomPlane(Vector3 center, float distance, float index)
		{
			return new Vector3
			{
				x = center.x + 0.035f * index - 0.127f,
				y = center.y - 0.003f,
				z = center.z + distance - 0.40f
			};
